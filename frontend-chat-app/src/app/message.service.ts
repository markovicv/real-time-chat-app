...
export class MessageService {
  ...
  sendMessage(messageToSend:String  ){
    ...
    this.httpClient.post<ChatMessage>("http://localhost:8080/message/chat",msg).subscribe(data=>{
  ...
  getAllFriendsList(){
    ...
    this.httpClient.get<any>("http://localhost:8080/message/friends/"+currentUsernameId).subscribe(data=>{
  ...
  getAllMessagesBetweenUsers(senderId:number,receiverId:number):Observable<any>{
    return this.httpClient.get<any>("http://localhost:8080/message/messages/"+receiverId+"/"+senderId);
  }
